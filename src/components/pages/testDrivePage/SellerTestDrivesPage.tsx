...
import { CalendarCheck, Check, Pencil, Trash2, X } from "lucide-react";
...
export default function SellerTestDrivesPage() {
  ...
  const { data: seller, isLoading: isLoadingSeller } = useGetSellerByUserIdQuery(
    userId,
    { skip: !userId }
  );
  ...
  const { data, isLoading: isLoadingTestDrives, error } = useGetTestDriveAllQuery(
  ...
  const filtered = useMemo(() => {
    if (!statusFilter) return allTestDrives;
    return allTestDrives.filter((item) => item.statusTestDrive === statusFilter);
  }, [allTestDrives, statusFilter]);

  const stats = useMemo(() => {
    const total = allTestDrives.length;
    const pending = allTestDrives.filter(
      (item) => item.statusTestDrive === StatusTestDrive.Pending
    ).length;
    const confirmed = allTestDrives.filter(
      (item) => item.statusTestDrive === StatusTestDrive.Confirmed
    ).length;
    const canceled = allTestDrives.filter(
      (item) => item.statusTestDrive === StatusTestDrive.Cancel
    ).length;
  ...
  const isLoading = isLoadingSeller || isLoadingTestDrives;
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4 md:px-10">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">จัดการทดลองขับ</h1>
        <p className="text-gray-500 text-sm mt-1">รายการนัดทดลองขับทั้งหมดของร้านคุณ</p>
      </div>
                ...
                {pagedTestDrives.map((item, i) => {
                  ...
                  const mainImage =
                    item.car?.carImages?.length
                      ...
                      : "/placeholder.png";
                  const canDelete = item.statusTestDrive === StatusTestDrive.Cancel;
                      ...
                      <td>
                        <div className="flex flex-col text-xs gap-0.5">
                          ...
                          <span className="text-gray-400 font-mono">
                            {item.car?.carRegistrationNumber ?? "-"}
                          </span>
                        </div>
                      </td>
                      <td>
                        <div className="flex flex-col text-xs gap-0.5">
                          ...
                          <span className="text-gray-400">{item.user?.phoneNumber ?? "-"}</span>
